...
def raceranks(races, startval=1500):
    ...
    for r in range(len(races)):
        ...
        for player in race["forfeits"]:
            match.addPlayer(player, min(11, len(race["finishers"])+1), ranks[r].get(player, startval))
        match.calculateELOs()
...
if __name__ == "__main__":
    ...
    #import pprint
    import rasterlogic
    #pprint.pprint([rasterlogic.numbered_byscore(r) for r in raceranks(races.races)])
    import sys
    rs = races.races
